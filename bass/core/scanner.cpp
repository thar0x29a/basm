namespace Bass {
  ...
  auto Scanner::scanTokens() -> vector<Token> {
    ...
    tokens.append({uid, TokenType::END, "EOF", nothing, line});
    return tokens;
  }
  ...
  auto Scanner::addToken(TokenType type, any literal) -> void {
    ...
    tokens.append({uid, type, text, literal, line});
  }
  ...
  auto Scanner::debug(const vector<Token>& tokens) -> void {
    for(Token token : tokens) {
      ...
      print("[ ", token.content, " ]");
      if(token.literal.isInt()) print("=", token.literal.getInt(), " ");
      if(token.literal.isFloat()) print("=", token.literal.getFloat(), " ");      
      //if(token.type == TokenType::kNAMESPACE) print("!");
      /**/
    }
  }
};